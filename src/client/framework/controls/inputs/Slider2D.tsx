...
function Slider2D({ value, min, max, onChange }: Slider2dProps) {
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent) => {
      const { left: parentLeft, top: parentTop } = e.currentTarget.getBoundingClientRect()

      const newValue = screenToValue(
        new V2(e.clientX - parentLeft - handleSize / 2, e.clientY - parentTop - handleSize / 2)
      )
      onChange?.(newValue)
    },
    [onChange, screenToValue]
  ...
  return (
    <>
      <div style={styles.gridContainer}>
        ...
        <GridLabel vec={V2.from(min.x, max.y)} vertical="bottom" horizontal="left" />
        <GridLabel vec={V2.from(max.x, min.y)} vertical="top" horizontal="right" />
      </div>
